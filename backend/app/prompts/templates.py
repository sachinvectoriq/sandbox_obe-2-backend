...
- Cite a Vetted Result by its short Citation Token in curly braces. Each Vetted Result above shows a token of the form `ref-N` (e.g. `ref-1`, `ref-2`, `ref-3`).
- Cite by writing the token in curly braces directly after the period: `{{ref-1}}`, `{{ref-2}}`, etc. NEVER invent a different token.
- The same Citation Token can be cited multiple times throughout your answer.
- If a sentence is supported by multiple Vetted Results, place each token in its own curly braces back-to-back, e.g. `{{ref-1}}{{ref-3}}`.
- NEVER place citations in the middle of a sentence — only at the end after the period.
- Before finalizing your answer, re-read it and verify that EVERY sentence ends with at least one `{{ref-N}}` citation. If any sentence is missing a citation, either add one from the Vetted Results or delete that sentence.

Example: "Azure Cosmos DB supports multiple APIs.{{ref-1}} It provides global distribution.{{ref-2}} It also offers multi-region writes.{{ref-1}}{{ref-3}}"
